...
function validateEmptyQuestionText(text) {
    if (text === null || text.length === 0) {
        BootstrapDialog.show({
            type: BootstrapDialog.TYPE_WARNING,
            title: 'Information',
            message: 'You can\'t leave the Question text empty!',
            buttons: [{
                label: 'Ok',
...
// loads the MathJax inside an element
function reloadMathJax(target) {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, target]);
}

// reloads all the MathJax in a page
function reloadAllMathJax() {
